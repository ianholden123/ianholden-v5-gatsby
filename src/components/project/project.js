    ...
    <MDXProvider components={components}>
      ...
      {isPostArchive && (
        <>
          ...
          <p
            className="uppercase f-thin color-dark-grey m-0"
        ...
        <ul className="meta ml-0">
          ...
          {contributors && (
            <li className="inline-block">
              <strong>Collaborators:</strong> {contributors}
            </li>
          )}
        </ul>
      )}
      {excerpt && (
        <div
          className="excerpt pb-4 reading-content block-center"
          dangerouslySetInnerHTML={{ __html: excerpt }}
        />
      )}
      {!isPostArchive && toolsUsed && renderTools(toolsUsed)}
      {!isPostArchive && colours && (
        <ul className="colours reading-content block-center text-center">
          {colours.primary && (
            <li className="inline-block mx-4">
              <div
                className="colour block-center"
          ...
          {colours.tertiary && (
            <li className="inline-block mx-4">
              <div
                className="colour block-center"
                style={{ backgroundColor: colours.tertiary }}
              />
              {colours.tertiary}
            </li>
          )}
